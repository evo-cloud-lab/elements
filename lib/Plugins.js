...
var ModulePlugin = Class({
    ...
    create: function () {
        ...
        try {
            plugin = require(moduleFile);
        } catch (e) {
            // ignored
        }

        if (plugin && typeof(object) == 'string') {
            plugin = plugin[object];
        }

        this.create = plugin && typeof(plugin.create) == 'function' ? plugin.create.bind(plugin) : function () { return null; };
...
var Plugins = Class({
     ...
     * @param plugin   Object provides 'create' method for instantiate the extension.
     *
     * @returns true if plugin is valid and registered, otherwise false.
     */
    register: function (extensionPoint, name, plugin) {
        if (plugin && typeof(plugin.create) == 'function') {
            ...
            exts.names[name] || exts.unshift(name);
            exts.names[name] = plugin;
            return true;
        }
        return false;
    },
    ...
    connect: function (host, extensionPoint, options) {
        ...
        var instantiate = function (names) {
            for (var i in names) {
                var name = names[i];
                var plugin = exts.names[name];
                var instance = plugin && plugin.create(data, host, extensionPoint, name);
        ...
        if (typeof(exts) == 'object') {
            for (var extPoint in exts) {
                ...
                for (var name in ext) {
                    ...
                    this.register(extPoint, name, new ModulePlugin(dir, extInfo));
                }
            }
        }
    }
