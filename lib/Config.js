...
function mergeOpts(opts, key, val) {
    if (typeof(val) == 'string') {
        if (opts[key] == undefined || opts[key] == null || typeof(opts[key]) == 'object') {
            ...
            opts[key] = opts[key].toString() + val;
        }
    } else if (typeof(opts[key]) == 'object') {
        mergeObject(opts[key], val);
    } else {
        opts[key] = val;
    }
}
...
function parseNormal(cfg, arg) {
    if (arg == '--') {
        ...
        if (v.key.length > 0) {
            if (!v.merge || cfg.opts[v.key] == undefined) {
                cfg.opts[v.key] = v.val;
            } else if (v.merge) {
                mergeOpts(cfg.opts, v.key, v.val);
            }
        }
    } else {
        cfg.args.push(arg);
