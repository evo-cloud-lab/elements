...
function applyAssignment(cfg, v) {
    if (v.key.length > 0) {
        var opts = cfg.opts, last;
        if (v.key == '.') { // '.' is reserved for root
            last = { opts: cfg, name: 'opts' };
        } else {
            var names = v.key.split('.');
...
var Config = Class(process.EventEmitter, {
    ...
    statics: {
        ...
        loadFileSync: function (filename, opts) {
            // TODO opts not supported
            return loadConfFile(filename);
        },

        loadFile: function (filename, opts, callback) {
            // TODO opts not supported
